...
function normalizeItems(payload: unknown): ApiItem[] {
  ...
  if (payload && typeof payload === "object") {
    const maybeItems =
      (payload as { itens?: unknown; items?: unknown }).itens ??
...
export default function HomePage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      if (!response) {
        throw new Error(
          "Não foi possível encontrar o endpoint de itens na API.",
        );
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-8 md:px-8">
      <main className="mx-auto w-full max-w-6xl space-y-6">
        <header className="rounded-2xl bg-linear-to-r from-blue-600 to-indigo-700 p-6 text-white shadow-lg">
              ...
              <table className="min-w-full divide-y divide-slate-200">
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {items.map((item, index) => {
                    ...
                    const nome =
                      item.nome ?? item.item ?? item.descricao ?? "Sem nome";
